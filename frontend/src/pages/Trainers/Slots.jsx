      ...
      <td className="px-6 py-4">
        <span
          className={`inline-flex items-center gap-1 rounded-full bg-black px-2 py-1 text-xs font-semibold 
            ${isSlotExpired(slot) && !slot.isBooked ? 'text-red-600 border border-red-600' : slot.isBooked ? 'text-yellow-600 border border-yellow-600' : 'text-green-600 border border-green-600'}`}
        >
          <span className={`h-1.5 w-1.5 rounded-full 
            ${isSlotExpired(slot) && !slot.isBooked ? 'bg-red-500' : slot.isBooked ? 'bg-yellow-500' : 'bg-green-400'}`}></span>
          {isSlotExpired(slot) && !slot.isBooked ? 'Expired' : slot.isBooked ? 'Booked' : 'Available'}
        </span>
      </td>
