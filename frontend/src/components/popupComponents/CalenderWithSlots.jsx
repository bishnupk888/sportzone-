...
const CalendarWithSlots = ({ slots, setIsOpen, trainerFee, trainerId }) => {
  ...
  useEffect(() => {
    const AvailableSlotsDates = slots.map((slot) =>
          ...
          {days.map((day, index) => (
            <th key={index}>
              <div className="w-full flex justify-center mb-4 ">
                <p className="text-lg font-large text-center text-white dark:text-white">
                  {day}
                </p>
              </div>
            </th>
  ...
  const renderCells = () => {
    ...
    while (day <= endDate) {
      for (let i = 0; i < 7; i++) {
        ...
        const isIncludeSlots = slotAvailableDates.includes(
          formatDate(new Date(cloneDay))
        );
            ...
            >
              <p
                className={`text-base font-medium ${
                  isSameDay(day, today) ? "text-white" : ""
                }`}
              >
                {formattedDate}
              </p>
    ...
    <div>
      ...
      <div className="absolute bottom-6 right-10 space-x-4">
        <button
          ...
          className=" text-white px-4 py-2 rounded-lg text-lg hover:scale-110 hover:text-red-600  border border-red-600 bg-red-700 hover:bg-black"
        >
          Cancel
        </button>
        {viewBookingSlots && (
          <button
            ...
            className="text-white px-4 py-2 rounded-lg text-lg hover:scale-110 hover:text-green-500  border border-green-600 bg-green-600 hover:bg-black"
          >
            Book Now
          </button>
        )}
